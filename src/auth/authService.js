...
class AuthService extends EventEmitter {
    ...
    isAuthenticated () {
      console.log('accesToken .',  localStorage.getItem('accessToken') , 'expiryToken', localStorage.getItem(tokenExpiryKey))
      console.log( 'date_local',new Date(Date.now()) , 'date_server', new Date(localStorage.getItem(tokenExpiryKey)) )
      console.log( parseInt(new Date(Date.now()).getTime())  / 1000 , parseInt(localStorage.getItem(tokenExpiryKey))  )

      return (
        parseInt(new Date(Date.now()).getTime()  / 1000 ) < parseInt(localStorage.getItem(tokenExpiryKey)) 
      )
    }
}
