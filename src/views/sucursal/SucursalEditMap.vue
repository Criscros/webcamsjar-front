...
export default {
  ...
  created(){
    ...
    setTimeout(()=>{
      //console.log('Edit map',this.$store.state['lat_lng'])
      this.center=  { lat: this.$store.state['lat_lng'].position.lat , lng:this.$store.state['lat_lng'].position.lng}
      this.markers.push( { position: { lat: this.$store.state['lat_lng'].position.lat, lng: this.$store.state['lat_lng'].position.lng}})
       this.zoom = 12
    },1000)
